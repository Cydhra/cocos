...
use anyhow::anyhow;
use libcocos::SiteLikelihoods;

/// Parse a puzzle-style site-loglikelihood
pub fn parse_puzzle<R: Read>(mut reader: BufReader<R>) -> anyhow::Result<Box<[Box<SiteLikelihoods>]>> {
    ...
    let num_trees: u32 = head_iter.next().ok_or(anyhow!("Invalid puzzle header."))?.parse()?;
    let num_sites: u32 = head_iter.next().ok_or(anyhow!("Invalid puzzle header."))?.trim().parse()?;

    let mut line = String::with_capacity(num_sites as usize * 10);
    let mut result = Vec::with_capacity(num_trees as usize);
    ...
    for i in 0..num_trees {
        ...
        if size > 0 {
            ...
            let _record_name = record.next().ok_or(anyhow!("Invalid puzzle record: no data"))?;
            let data = record.next().ok_or(anyhow!("Invalid puzzle record: no data"))?.trim();

            // split the rest of the columns
            let sites_iter = data.splitn(num_sites as usize, ' ');
            let site_lh = sites_iter
                .map(|site| site.parse())
                .collect::<Result<Box<[f64]>, _>>()
                .map_err(|e| anyhow!("Failed to parse site Likelihoods of tree {}: {}", i, e))?;
            result.push(site_lh);
        } else {
...
mod tests {
    ...
    fn test_simple_puzzle() {
        ...
        assert!(result.is_ok(), "Parsing puzzle failed: {}", result.unwrap_err());
